...
RISK_PCT       = float(os.environ.get("RISK_PCT", 2))  # % of equity risked per trade
...
def open_position(state, symbol, base, direction, price, sl, tp1, atrV, score, witnesses, equity):
    ...
    try:
        ...
        # Take-profit order (TP1, 100% close)
        client.futures_create_order(
    ...
    if pos["trail_active"]:
        ...
        if should_update:
            ...
            try:
                ...
                client.futures_create_order(
                    symbol=symbol, side=opp_side, type="STOP_MARKET",
                    stopPrice=new_stop_r, closePosition=True,
                )
                client.futures_create_order(
                    symbol=symbol, side=opp_side, type="TAKE_PROFIT_MARKET",
                    stopPrice=pos["tp1"], closePosition=True,
                )
                pos["stop"] = new_stop_r
                save_state(state)
            except BinanceAPIException as e:
...
def main():
    ...
    tg_send(
        ...
        f"Leverage: {LEVERAGE}x | Risk: {RISK_PCT}%/trade\n"
